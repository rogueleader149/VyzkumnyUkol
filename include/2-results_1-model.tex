...
\subsection{Feasibility of the class $\BPP$}
	...
	\begin{conj}   %!% conjecture nebo lépe?
		Feasible DNA algorithms comply $Bs(n)\in O(1)$; $Bnd(n),\,Ti(n),\,Gl(n)\in \P$.   %!% možná postačí pro poly algoritmus mín jak \P tile a glue !!!
	\end{conj}
	...
	\begin{thm}   %!% nějak pojmenovat model (cca ?aTAM ptž z aTAM vychází ale zužuje), deterministic version by Winfree
	...
	\begin{cor}
		$\BPP$ is decidable by abstract model.   %!% pojmenovat model
	\end{cor}
	
	%!% pojmenovat model a nák rozumně vysvětlit že \NP se dá na PTM docela dobře spočítat
