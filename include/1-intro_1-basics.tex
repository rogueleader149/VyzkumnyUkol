...
\section{Complexity, languages}
	...
	\subsection{Studied complexities}
		...
		\label{rem:stud_compl}
			...
			\begin{description}
				...
				\item[Ad glue complexity] Higher glue complexity will require longer DNA sequences in the sticky ends. % which leads to higher probability of erroneous binding $p_e$. %!% něco citovat co zminuje závislost p_e na dýlce sekvence, to přece musí rost ...
			\end{description}
		\end{remark}
	...
	\label{sec:PNP}
		...
		\begin{example}\label{exm:npc}
			Remind some popular $\NPC$ problems: boolean formula satisfability ($\sf SAT$), Hamiltonian path problem ($\sf HPP$), graph $3$-coloring, graph $k$-independent set, graph $k$-clique, graph $k$-vertex cover, subset sum and many others.
			
			On the other hand there are few interesting $\NP$ problems which are not known to belong either to $\P$ nor to $\NPC$, one of them is {\em graph isomorphism problem}.
		\end{example}
		...
		\begin{conj}
			It commonly assumed that these unproven relations hold.
			\begin{align*}
				\P &\subsetneq \NP, \\
				\P &= \BPP.
			\end{align*}
