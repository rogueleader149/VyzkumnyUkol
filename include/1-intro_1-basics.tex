...
\section{Complexity, languages}
	...
	\label{sec:PNP}
		...
		There exist many equivalent definitions, these are taken from \cite{book_comp}. The first set of definitions is very straightforward and thus can be used for propositions.
		...
		We say that PTM $M$ decides language $\cal L$ iff for every $x\in\Sigma^*$ the probability of halting in correct state (i.e. $q_{accept}$ for $x\in{\cal L}$ and $q_{reject}$ for $x\notin{\cal L}$) is higher than $\nicefrac{2}{3}$.
		
		We say that PTM $M$ decides language $\cal L$ in time $t(n)$ iff $M$ decides language $\cal L$ and for every $x\in\Sigma^*$ it halts in time $\leq t(|x|)$. Now we can define those classes, note that it would be possible to use again two types of definition like above.
		...
		\begin{remark}
			$\ZPP$ does not allow PTM to make any error, still it can halt without accepting nor rejecting.
		\end{remark}
		...
		%!% ZPP .. book pg 125, tam jsou i theorémy
		
		% P, NP, co-NP, PP, \#P, PSpace. Enough? Maybe also polynomial hierarchy: $\Sigma_k P$ and $\Pi_k P$ languages (alternating Turing machine with bounded alternation, \cite{kozen06}). Might be nice to show what DNA computation is capable of.
