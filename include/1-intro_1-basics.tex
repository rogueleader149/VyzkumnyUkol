...
\section{Complexity, languages}
	...
	\subsection{Languages}
		...
		\begin{remark}
			Note that given an alphabet $\Sigma$ the set of all words $\Sigma^*$ is countable. Moreover, one can sort $\Sigma^*$ lexicographically thus it is easy to define a bijection $f: \N \leftrightarrow \Sigma^* $.
	...
	\label{sec:PNP}
		...
		The second possibility is to first define classes $\DTime$, $\NTime$, $\DSpace$ and $\NSpace$ and after that to use their special case.
		
		%!% někde najít dobrej zdroj
		% tady je problém s negací v D---- ptž polynom časově konstruovatelnej je ale co když f(n) neni že ...
		\begin{defn}
